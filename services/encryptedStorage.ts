...
async function streamXOR(keyHex: string, ivHex: string, inputHex: string): Promise<string> {
  ...
  for (let i = 0; i < inputBytes.length; i++) {
    if (keyStream.length === 0) {
      ...
      blockSeed = await getCrypto().digestStringAsync(getCrypto().CryptoDigestAlgorithm.SHA256, blockSeed);
...
// Uses expo-crypto.getRandomBytesAsync for CSPRNG entropy.
// Fallback: XOR of four sources (time, Math.random x2, performance.now)
// when expo-crypto is unavailable — still an improvement over a single
// predictable seed, but callers should prefer the primary path.
async function randomIV(): Promise<string> {
  try {
    const c = getCrypto();
    if (c?.getRandomBytesAsync) {
      const bytes: Uint8Array = await c.getRandomBytesAsync(16);
      return Array.from(bytes).map(b => b.toString(16).padStart(2, '0')).join('');
    }
  } catch {}
  // Fallback: blend 4 independent entropy sources and hash
  const e1 = Date.now().toString(36);
  const e2 = Math.random().toString(36).slice(2);
  const e3 = Math.random().toString(36).slice(2);
  const e4 = (typeof performance !== 'undefined' ? performance.now() : Date.now() ^ 0xABCD).toString(36);
  const seed = `${e1}-${e2}-${e3}-${e4}-butler-iv`;
  const hash = await getCrypto().digestStringAsync(getCrypto().CryptoDigestAlgorithm.SHA256, seed);
...
class EncryptedStorage {
   ...
   *
   * Call from app boot (nexusIntegrityEngine.runScan) after encryptedStorage
