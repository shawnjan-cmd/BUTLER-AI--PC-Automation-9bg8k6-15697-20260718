...
class AutoConnectEngine {
  ...
  notifyConnected(ip: string, port: string, latencyMs = 0): void {
    ...
    AsyncStorage.multiSet([[KEYS.IP, ip], [KEYS.PORT, String(port)]]).catch(() => {});
    ...
    if (savedIp && savedPort) {
      ...
      if (fastest) {
        ...
        if (r2?.connected) {
          ...
          this._failStreak = 0;
          await AsyncStorage.multiSet([[KEYS.IP, fastest.ip],[KEYS.PORT, String(fastest.port)]]).catch(() => {});
  ...
  private async _connectToDiscovered(ip: string, port: string, isFirstTime: boolean): Promise<void> {
    ...
    try {
      ...
      if (r.connected) {
        ...
        await AsyncStorage.multiSet([[KEYS.IP, ip],[KEYS.PORT, String(resolvedPort)]]).catch(() => {});
  ...
  private _startPersistentRetry(ip: string, port: string): void {
    ...
    this._persistRetryTimer = setInterval(async () => {
      ...
      if (ms !== null) {
        ...
        this._lastIp   = ip;
        this._lastPort = port;
        await AsyncStorage.multiSet([[KEYS.IP, ip],[KEYS.PORT, String(port)]]).catch(() => {});
...
// ── IMPORTANT: This file is an alias of autoConnectEngine.ts ──────────────────
// The canonical singleton lives in services/autoConnectEngine.ts.
// Do NOT create a second instance here — it would shadow the real singleton
// and cause two independent state machines to run simultaneously.
// Import from autoConnectEngine.ts instead:
