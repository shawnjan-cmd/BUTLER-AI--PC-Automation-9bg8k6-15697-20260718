...
class PCClipboardService {
  ...
  async syncBidirectional(): Promise<{ ok: boolean; text: string }> {
    ...
    try {
      const getStringAsync = () => new Promise<string>((res) => { try { require('react-native').Clipboard.getString().then(res).catch(() => res('')); } catch { res(''); } });
      const setStringAsync = (s: string) => { try { require('react-native').Clipboard.setString(s); } catch {} return Promise.resolve();};
