...
class DeviceIdentifierService {
  ...
  async getDeviceId(): Promise<string> {
    ...
    try {
      ...
      // 4. Fallback: CSPRNG-generated ID persisted for this install.
      if (!id) {
        const crypto = await import('expo-crypto');
        const bytes = await crypto.getRandomBytesAsync(16);
        id = 'butler-rnd-' + Array.from(bytes).map(b => b.toString(16).padStart(2, '0')).join('');
        await AsyncStorage.setItem(DEVICE_ID_KEY, id).catch(() => {});
      }
      ...
      // 5. Complete storage failure — fail closed rather than inventing predictable identity material.
      if (!this.deviceId) {
        this.deviceId = null;
      }
      throw new Error('Unable to create a cryptographically random device identifier');
    }
  }
