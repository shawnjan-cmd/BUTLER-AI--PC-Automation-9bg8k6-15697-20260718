/**
 * BUTLER SCRIPT TEMPLATES — Categorized Python automation scripts
...
print("[ BUTLER PASSWORD GENERATOR ]")
...
for name, url in apis:
    try:
        req = urllib.request.Request(url, headers={'User-Agent':'ButlerBot/1.0'})
        with urllib.request.urlopen(req, timeout=5) as r:
            data = json.load(r)
        print(f"[ {name} ]")
...
try:
  ...
  // ─── UNIQUE / EXCLUSIVE ──────────────────────────────────────────
  {
    ...
    code: `# BUTLER SEQUENCER v1.0 — Run scripts in sequence
...
print(json.dumps(results, indent=2))`,
  },
  {
    ...
    code: `# BUTLER DELTA OBSERVER — File system change detector
