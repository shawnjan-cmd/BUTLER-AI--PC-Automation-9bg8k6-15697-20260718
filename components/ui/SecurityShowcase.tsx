...
export const SECURITY_FACTS: SecurityFact[] = [
  ...
  { icon: 'lock',           mcIcon: 'lock-outline',         color: C.cyan,   label: 'AES-256-GCM',     detail: 'Sensitive app data uses authenticated AES-256-GCM at rest. Transport security depends on the configured local or tunnel connection.',           tab: 'settings',  route: '/(tabs)/settings'  },
  ...
  { icon: 'storage',        mcIcon: 'database-lock',        color: C.cyan,   label: 'LOCAL STORAGE',   detail: 'Sensitive app data is stored with authenticated encryption on-device. Chat and memory remain on the paired PC; no cloud database is required.',  tab: 'knowledge', route: '/(tabs)/knowledge' },
  { icon: 'memory',         mcIcon: 'chip',                 color: C.amber,  label: 'LOCAL AI MODEL',  detail: 'Ollama runs on your own PC. The app has no cloud-AI fallback and does not send chat to third-party model providers.',         tab: 'butler',    route: '/(tabs)/butler'    },
  { icon: 'qr-code',        mcIcon: 'qrcode-scan',          color: C.blue,   label: 'QR PAIRING',      detail: 'Pairing is one-scan with a QR code from butler_server.py. No account, no signup.',                tab: 'home', route: '/(tabs)/home' },
  { icon: 'verified',       mcIcon: 'check-decagram',       color: C.green,  label: 'SECURITY CHECKS', detail: 'The app reports connection, token, transport, and server-lock status from live checks; unresolved states are shown instead of guessed.',   tab: 'settings',  route: '/(tabs)/settings'  },
