...
function Particle({ style, color, index, opacity, active }: ParticleProps) {
  ...
  useEffect(() => {
    ...
    const loop = Animated.loop(Animated.sequence([
      ...
      Animated.parallel([
        ...
        Animated.sequence([
          Animated.timing(fadeA, { toValue: opacity, duration: 500, useNativeDriver: true }),
          Animated.delay(seed.dur - 1000),
          Animated.timing(fadeA, { toValue: 0,       duration: 500, useNativeDriver: true }),
        ]),
      ]),
  ...
  // Split into two nested Animated.Views:
  // outer: JS driver (top position — cannot run on native)
  // inner: native driver (opacity only — can run on native)
  return (
    <Animated.View style={{
      position: 'absolute',
      left: seed.startX,
