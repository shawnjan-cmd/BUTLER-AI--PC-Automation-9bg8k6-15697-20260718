...
export function MessageActionBar({
  ...
  const doCopy = async () => {
    safeH.light();
    await safeSetClipboard(text); setCopied(true); setTimeout(() => setCopied(false), 1400);
  };
  const doShare = async () => {
    safeH.light();
...
function CodeBlock({ code, lang }: { code: string; lang?: string }) {
  const [copied, setCopied] = useState(false);
  const copy = async () => { await safeSetClipboard(code); setCopied(true); setTimeout(() => setCopied(false), 1200); safeH.light(); };
  return (
