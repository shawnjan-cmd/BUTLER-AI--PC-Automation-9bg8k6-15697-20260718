 ...
 * Auto-starts on mount. Styled to Butler Omega design system.
 */
...
// serverConnection is accessed lazily inside callbacks to avoid crashes
// when this module is loaded via dynamic import() from nexushome.tsx.
// A top-level import causes "Cannot destructure property 'serverIp' of null"
// because the serverConnection singleton may not be ready at module-eval time.
let _serverConn: any = null;
function getServerConn() {
  if (_serverConn) return _serverConn;
  try { _serverConn = require('@/services/serverConnection').serverConnection; } catch { _serverConn = null; }
  return _serverConn;
}
...
export function OmegaLearningLoop({
  ...
  const fetchOmegaKnowledge = useCallback(async () => {
    const sc   = getServerConn();
    const ip   = serverIp   || sc?.getIP?.()   || '';
    const port = serverPort || sc?.getPort?.() || '';
    ...
    try {
      ...
      const sc2  = getServerConn();
      const tok  = token || sc2?.getToken?.() || '';
      ...
      if (!res.ok) {
        ...
        try {
          const res2 = await fetch(`http://${ip}:${port}/api/learn/status`, { headers, signal: (() => { const c = new AbortController(); setTimeout(() => c.abort(), 5000); return c.signal; })() });
