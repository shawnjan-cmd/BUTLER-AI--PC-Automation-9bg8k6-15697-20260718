...
import {
  ...
  StyleSheet, Platform, Alert, ActivityIndicator, Animated, Modal, Dimensions, FlatList, PanResponder,
} from 'react-native';
...
import { NexusLiveCard } from '@/components/ui/NexusLiveCard';
import { IsometricKBShardMap } from '@/components/cyber/IsometricKBMap';
import { perf } from '@/services/performanceTuner';
...
// ─── KB LIVE DASH CARD (DataStatCard pattern) ───────────────────
function KBLiveDashCard({ isConnected }: { isConnected: boolean }) {
  const [kbVal,      setKbVal]      = useState(0);
  const [sources,    setSources]    = useState(0);
  const [sessions,   setSessions]   = useState(0);
  const [kbHistory,  setKbHistory]  = useState<number[]>([]);
  const pulseAnim = useRef(new Animated.Value(0.4)).current;

  useEffect(() => {
    // Try to load from server, fall back to local KB stats
    const load = async () => {
      try {
        const { serverConnection } = await import('@/services/serverConnection');
        const ip = serverConnection.getIP?.();
        const port = serverConnection.getPort?.();
        if (ip && port && isConnected) {
          const token = serverConnection.getToken?.();
          const h: Record<string,string> = {};
          if (token) h['Authorization'] = `Bearer ${token}`;
          const _kbCtrl = new AbortController(); const _kbT = setTimeout(() => _kbCtrl.abort(), 5000);
          const res = await fetch(`http://${ip}:${port}/api/learn/status`, { headers: h, signal: _kbCtrl.signal }).finally(() => clearTimeout(_kbT));
          if (res.ok) {
            const d = await res.json();
            const articles = d.articlesTotal ?? d.vectorCount ?? d.total ?? 0;
            setKbVal(articles);
            setSources(d.sourcesCount ?? d.sources ?? 0);
            setSessions(d.sessions ?? d.totalSessions ?? 0);
            setKbHistory(prev => [...prev.slice(-19), articles > 1000 ? Math.round(articles / 100) : articles]);
            return;
          }
        }
      } catch {}
      // fallback: local knowledge accumulator
      try {
        const { knowledgeAccumulator } = await import('@/services/knowledgeAccumulator');
        const stats = await knowledgeAccumulator?.getStats?.();
        if (stats) {
          setKbVal(stats.totalFindings ?? 0);
  ...
  const sparkPoints = useMemo(() => {
    if (kbHistory.length > 2) return kbHistory;
    const base = kbVal > 0 ? Math.max(5, Math.min(kbVal, 100)) : 10;
    return Array.from({ length: 20 }, (_, i) => Math.max(1, base * (0.4 + (i / 20) * 0.6)));
  }, [kbHistory, kbVal]);

  const statusColor = isConnected ? N.green : N.amber;
  const valueLabel = kbVal > 9999 ? `${(kbVal/1000).toFixed(1)}K`
    : kbVal > 999 ? `${(kbVal/1000).toFixed(1)}K` : String(kbVal || 0);

  return (
    <View style={[kbc.card, { backgroundColor: N.surface, borderColor: N.amber + '25' }]}>
      <View style={[kbc.topBar, { backgroundColor: N.amber }]} />
      <View style={kbc.header}>
        <View style={[kbc.iconBox, { backgroundColor: N.amber + '18', borderColor: N.amber + '40' }]}>
          <MaterialCommunityIcons name="brain" size={16} color={N.amber} />
        </View>
        <View style={{ flex: 1 }}>
          <Text style={kbc.title}>Knowledge Base</Text>
          <Text style={kbc.subtitle}>NEURAL VECTOR STORE</Text>
        </View>
        <View style={[kbc.statusBadge, { borderColor: statusColor + '55', backgroundColor: statusColor + '12' }]}>
          <Animated.View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: statusColor, opacity: pulseAnim }} />
          <Text style={[kbc.statusTxt, { color: statusColor }]}>{kbVal > 0 ? 'HARVESTING' : 'IDLE'}</Text>
        </View>
      </View>
      <Text style={[kbc.bigNum, { color: N.amber }]} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.5}>
        {valueLabel}
      </Text>
      <View style={{ paddingHorizontal: 14, paddingBottom: 10 }}>
        <View style={{ height: 48, flexDirection: 'row', alignItems: 'flex-end', gap: 1.5, overflow: 'hidden' }}>
          {sparkPoints.map((pt, i) => {
            const maxPt = Math.max(...sparkPoints, 1);
            const h = Math.max(3, (pt / maxPt) * 44);
            const isLast = i === sparkPoints.length - 1;
            return (
              <View key={i} style={[{
                flex: 1, borderRadius: 2, backgroundColor: N.amber,
                height: h, opacity: isLast ? 1 : 0.3 + (i / sparkPoints.length) * 0.65,
              }, Platform.OS === 'ios' && isLast ? { shadowColor: N.amber, shadowOffset:{width:0,height:0}, shadowOpacity:0.9, shadowRadius:8 } : {}]} />
            );
          })}
        </View>
      </View>
      <View style={kbc.statRow}>
        {[
          { label: 'SOURCES',  value: sources > 0 ? String(sources) : '--' },
          { label: 'SESSIONS', value: sessions > 0 ? String(sessions) : '--' },
          { label: 'STATUS',   value: isConnected ? 'LIVE' : 'LOCAL' },
        ].map((s, i) => (
          <View key={i} style={[kbc.statCell, i < 2 && { borderRightWidth: 1, borderRightColor: 'rgba(255,255,255,0.07)' }]}>
            <Text style={kbc.statLabel}>{s.label}</Text>
            <Text style={[kbc.statValue, { color: i === 2 ? (isConnected ? N.green : N.amber) : N.text }]}>{s.value}</Text>
          </View>
        ))}
      </View>
    </View>
...
const kbc = StyleSheet.create({
  ...
  title:      { fontSize: 15, fontWeight: '900', fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace', color: '#D2E8F6', letterSpacing: 0.3 },
  subtitle:   { fontSize: 9, fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace', letterSpacing: 1.2, marginTop: 2, color: '#6890A8' },
  ...
  statusTxt:  { fontSize: 9, fontWeight: '900', fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace', letterSpacing: 0.5 },
  bigNum:     { fontSize: 64, fontWeight: '900', fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace', paddingHorizontal: 14, marginBottom: 6, lineHeight: 70 },
  ...
  statLabel:  { fontSize: 8, fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace', fontWeight: '700', letterSpacing: 1.2, marginBottom: 4, color: '#6890A8' },
  statValue:  { fontSize: 16, fontWeight: '900', fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace' },
});
...
function KBIntelDashboard({ isConnected }: { isConnected: boolean }) {
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 12, paddingBottom: 140, gap: 10 }} showsVerticalScrollIndicator={false}>
      {/* KPI cards — 3-col grid (LAYOUT LAW) */}
      {(() => {
        const COL3 = Math.floor((SW - 28 - 8 * 2) / 3);
        return (
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
            {[
              { label: 'KB ARTICLES',  val: articles,                  sub: 'SYNCED',        col: '#00C2FF' },
              { label: 'SOURCES',      val: liveData?.queuePending ?? localStats.sources, sub: 'ENDPOINTS', col: '#00FF88' },
              { label: 'SCRIPTS RUN',  val: localStats.scriptsRun,     sub: 'EXECUTED',      col: '#FFAA00' },
              { label: 'TOPIC GAPS',   val: localStats.gaps,           sub: localStats.gaps > 0 ? 'Filling...' : 'Covered', col: '#FF5566' },
              { label: 'WORKERS',      val: liveData?.workersRunning ?? 0, sub: 'ACTIVE',    col: accent   },
              { label: 'QUEUE',        val: liveData?.queuePending ?? 0,   sub: 'PENDING',   col: N.sigma  },
            ].map(({ label, val, sub, col }) => (
              <View key={label} style={[kbid.kpiCard, { borderColor: col + '55', width: COL3 }]}>
                <View style={[kbid.kpiTopBar, { backgroundColor: col }]} />
                <View style={{ paddingTop: 8, gap: 3 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 10 }}>
                    <Text style={[kbid.kpiLabel, { color: '#5A6A7A' }]} numberOfLines={1}>{label}</Text>
                    <Animated.View style={{ width: 5, height: 5, borderRadius: 3, backgroundColor: col, opacity: liveDot }} />
                  </View>
                  <Text style={[kbid.kpiVal, { color: col, fontSize: 28, paddingHorizontal: 10 }]} adjustsFontSizeToFit minimumFontScale={0.5} numberOfLines={1}>{val}</Text>
                  <Text style={[kbid.kpiSub, { color: '#4A5A6A', paddingHorizontal: 10, paddingBottom: 8 }]} numberOfLines={1}>{sub}</Text>
                </View>
              </View>
            ))}
          </View>
...
const kbid = StyleSheet.create({
  kpiCard:   { backgroundColor: '#07101A', borderWidth: 1.5, borderRadius: 12, minHeight: 90, position: 'relative', overflow: 'hidden', padding: 0 },
  kpiTopBar: { height: 3, borderTopLeftRadius: 12, borderTopRightRadius: 12 },
  kpiLabel:  { fontSize: 9, fontWeight: '700', fontFamily: MONO, letterSpacing: 0.8, paddingHorizontal: 12 },
  kpiVal:    { fontSize: 34, fontWeight: '900', fontFamily: MONO, lineHeight: 38, paddingHorizontal: 12 },
  kpiSub:    { fontSize: 9, fontWeight: '600', fontFamily: MONO, letterSpacing: 0.3, paddingHorizontal: 12, paddingBottom: 10 },
      ...
      {drives.length === 0 ? (
        <View style={lsc.emptyCard}>
          ...
          <Text style={lsc.emptyTxt}>No server connected</Text>
           <TouchableOpacity
             onPress={() => { try { require('react-native').Linking.openURL('https://github.com/shawnjan-cmd/butler-server/releases'); } catch {} }}
             style={{ marginTop:8, flexDirection:'row', alignItems:'center', gap:6, backgroundColor:'#00E5FF18', borderWidth:1.5, borderColor:'#00E5FF55', borderRadius:10, paddingHorizontal:12, paddingVertical:9 }}
             activeOpacity={0.8}
           >
             <MaterialCommunityIcons name="github" size={16} color="#00E5FF" />
             <Text style={{ fontFamily: Platform.OS==='ios'?'Menlo-Bold':'monospace', fontSize:10, fontWeight:'900', color:'#00E5FF', letterSpacing:0.5 }}>DOWNLOAD BUTLER SERVER ↗</Text>
           </TouchableOpacity>
...
// ── NEXUS BOT renamed to BUTLER BOT ─────────────────────────────────────────
// All "NEXUS BOT" references → "BUTLER BOT" in tab label and header
    ...
    <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 14, paddingBottom: 140, gap: 14 }} showsVerticalScrollIndicator={false}>
      ...
      <Animated.View style={[nbot.headerCard, {
        ...
        <View style={nbot.headerRow}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[nbot.botTitle, { color: AMBER }]}>[ BUTLER BOT ]</Text>
...
function KnowledgeScreenInner() {
  ...
  useEffect(() => {
    ...
    setIsConnected(seed.connected || serverConnection.isConnected());
  ...
  const TABS: { key: TabKey; label: string; icon: string; color: string }[] = [
    ...
    { key: 'nexusbot',  label: 'BUTLER BOT',  icon: 'smart-toy',       color: N.amber  },
  ...
  return (
    <View style={s.container}>
      ...
      <CompactPageHeader
        ...
        iconLib="community"
        title="KNOWLEDGE"
        badge={`${stats?.totalFindings ?? 0} FACTS`}
        badgeColor="#FFB020"
        isConnected={isConnected}
        safeTop={insets.top || 0}
        rightAction={{ icon: 'refresh', onPress: () => { haptics.light(); loadKB(); }, color: '#FFB020' }}
        extraRow={
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 12, paddingVertical: 6 }}>
            <PageMascot page="knowledge" size="sm" showBubble />
            <MaterialCommunityIcons name="brain" size={11} color="#FFB020" />
            <Text style={{ fontFamily: MONO, fontSize: 9, fontWeight: '900', color: '#FFB020', letterSpacing: 1 }}>AI MEMORY</Text>
            <View style={{ flex: 1, height: 3, borderRadius: 2, backgroundColor: '#FFB02018', overflow: 'hidden' }}>
              <View style={{ width: `${Math.min(100, Math.max(5, (stats?.totalFindings ?? 0) / 10))}%` as any, height: '100%', backgroundColor: '#FFB020', borderRadius: 2 }} />
            </View>
            <Text style={{ fontFamily: MONO, fontSize: 9, color: '#FFB02080' }}>{stats?.totalFindings ?? 0} facts</Text>
            <View style={{ flexDirection: 'row', gap: 4 }}>
              {['Py', 'Sys', 'Net', 'AI'].map((cat, i) => (
                <View key={cat} style={{ borderWidth: 1, borderRadius: 5, paddingHorizontal: 5, paddingVertical: 2,
                  borderColor: ['#00CCFF','#00FF88','#9D6FFF','#CC44FF'][i] + '50',
                  backgroundColor: ['#00CCFF','#00FF88','#9D6FFF','#CC44FF'][i] + '0A' }}>
                  <Text style={{ fontFamily: MONO, fontSize: 7, fontWeight: '900', color: ['#00CCFF','#00FF88','#9D6FFF','#CC44FF'][i] }}>{cat}</Text>
                </View>
              ))}
            </View>
          </View>
        ...
        <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 140 }}>
          ...
          <View style={{ marginHorizontal: 12, marginTop: 4, marginBottom: 4 }}>
            <NexusLiveCard
              ...
              subtitle="AI NEURAL STORE · LIVE"
              accent={N.amber}
              isLive={isConnected}
              stats={[
                { label: 'FINDINGS', value: stats?.totalFindings ?? 0, color: N.amber },
                { label: 'SESSIONS', value: stats?.totalSessions ?? 0, color: N.blue },
                { label: 'STORAGE',  value: stats ? `${(stats.storageUsed/1024).toFixed(0)}K` : '--', color: N.green },
              ]}
          ...
          <View style={{ marginHorizontal: 12, marginTop: 4 }}>
            <KBLiveDashCard isConnected={isConnected} />
          </View>
