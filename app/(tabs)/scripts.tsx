/**
 * ⚡ BUTLER SCRIPT LIB — Performance-optimized v3.0
...
const SCRIPTS_PAGE_TIPS: string[] = [
  ...
  'Tap ▶ to run a script on your PC.',
  'Star a script to pin it to the top.',
  'AI tab writes scripts from plain English.',
  'CHAIN sequences up to 20 scripts at once.',
  'Search matches names, tags & descriptions.',
  'AI scripts are saved permanently to library.',
  'Filter chips narrow by category (System/Web…)',
  'Output modal streams live PC results.',
  'Templates has 30 ready-to-use Python recipes.',
  'Phone is the remote — scripts run on your PC.',
  'Safety scanner checks 15 threat patterns first.',
  'Run count badge tracks total executions.',
  'Pinned scripts always appear at the top.',
];
...
async function copyCode(code: string) {
  try {
    ...
    else { try { require('react-native').Clipboard.setString(code); } catch {} }
  } catch {}
}
...
// ─── BUTLER SCRIPT CARD — 3D Cyberpunk Edition ───────────────────
...
function CategoryDetailModal({ cat, onClose }: { cat: CategoryDef | null; onClose: () => void }) {
  ...
  return (
    <Modal visible={!!cat} animationType="slide" statusBarTranslucent
      ...
      <View style={{ flex: 1, backgroundColor: '#060A10' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 12, paddingVertical: 8, backgroundColor: '#030810', borderBottomWidth: 1, borderBottomColor: '#00DCFF25', paddingTop: Math.max(insets.top, 0) + 8 }}>
              <MaterialCommunityIcons name="code-braces-box" size={14} color="#00DCFF" />
              <Text style={{ fontFamily: MONO, fontSize: 12, fontWeight: '900', color: '#00DCFF', letterSpacing: 1.5, flex: 1 }}>PYTHON AUTOMATION LIBRARY</Text>
              {['SYSTEM','FILES','AI'].map((label, i) => (
                <View key={label} style={{ borderWidth: 1, borderRadius: 5, paddingHorizontal: 5, paddingVertical: 2,
                  borderColor: ['#00DCFF','#FFB020','#CC44FF'][i] + '40',
                  backgroundColor: ['#00DCFF','#FFB020','#CC44FF'][i] + '08' }}>
                  <Text style={{ fontFamily: MONO, fontSize: 7, fontWeight: '900', color: ['#00DCFF','#FFB020','#CC44FF'][i] }}>{label}</Text>
                </View>
              ))}
            </View>
...
// ─── FUI TIP BANNER — expandable, tap to read full sentence ─────────
function FuiTipBanner({ tips }: { tips: string[] }) {
  ...
  useEffect(() => {
    if (expanded) return; // pause rotation while expanded
    const t = setInterval(() => {
      Animated.timing(fadeAnim, { toValue: 0, duration: 260, useNativeDriver: false }).start(() => {
  ...
  const handleNext = useCallback(() => {
    ...
    Animated.timing(fadeAnim, { toValue: 0, duration: 160, useNativeDriver: false }).start(() => {
      setIdx(i => (i + 1) % tips.length);
      Animated.timing(fadeAnim, { toValue: 1, duration: 160, useNativeDriver: false }).start();
    });
  }, [tips.length]);

  const maxH = expandAnim.interpolate({ inputRange: [0, 1], outputRange: [0, 80] });

  return (
    <TouchableOpacity onPress={handleToggle} activeOpacity={0.85} style={ftb.wrap}>
      <View style={ftb.accent} />
      <View style={{ flex: 1 }}>
        {/* Collapsed single-line preview */}
        <Animated.Text
          style={[ftb.txt, { opacity: fadeAnim }]}
          numberOfLines={expanded ? undefined : 1}
          ellipsizeMode="tail"
        >
          {tips[idx]}
        </Animated.Text>
        {/* Expanded full sentence + nav */}
        {expanded ? (
          <Animated.View style={{ overflow: 'hidden', maxHeight: maxH }}>
            <View style={ftb.expandRow}>
              <Text style={ftb.expandCounter}>{idx + 1} / {tips.length}</Text>
              <TouchableOpacity onPress={handleNext} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }} style={ftb.nextBtn}>
                <MaterialIcons name="arrow-forward" size={12} color={N.blue} />
                <Text style={ftb.nextTxt}>NEXT TIP</Text>
              </TouchableOpacity>
            </View>
          </Animated.View>
        ) : null}
      ...
      <View style={[ftb.chip]}>
        <MaterialIcons name={expanded ? 'keyboard-arrow-up' : 'lightbulb-outline'} size={10} color={N.blue} />
        <Text style={ftb.chipTxt}>{expanded ? 'LESS' : 'TIP'}</Text>
      </View>
    </TouchableOpacity>
  );
}
const ftb = StyleSheet.create({
  wrap:       { flexDirection: 'row', alignItems: 'stretch', marginHorizontal: 14, marginTop: 8, marginBottom: 2,
  ...
  txt:        { fontSize: 11, color: N.blue + 'CC', fontFamily: MONO, lineHeight: 16,
  ...
  expandRow:  { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
                paddingHorizontal: 10, paddingBottom: 9, gap: 8 },
  expandCounter: { fontSize: 9, color: N.blue + '70', fontFamily: MONO, letterSpacing: 0.5 },
  nextBtn:    { flexDirection: 'row', alignItems: 'center', gap: 4, borderWidth: 1,
                borderColor: N.blue + '40', borderRadius: 6, paddingHorizontal: 8, paddingVertical: 4,
                backgroundColor: N.blue + '10' },
  nextTxt:    { fontSize: 9, fontWeight: '900', color: N.blue, fontFamily: MONO, letterSpacing: 0.4 },
  chip:       { flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: N.blue + '15',
                borderLeftWidth: 1, borderLeftColor: N.blue + '30', paddingHorizontal: 10,
                justifyContent: 'center', flexShrink: 0 },
  chipTxt:    { fontSize: 9, fontWeight: '900', color: N.blue, fontFamily: MONO, letterSpacing: 0.5 },
});

// ─── SCRIPT STREAK TRACKER ────────────────────────────────────────────────────
const STREAK_KEY = 'butler_script_streak_v1';
async function updateScriptStreak(): Promise<{ streak: number; best: number; lastDate: string }> {
  try {
    const raw = await AsyncStorage.getItem(STREAK_KEY);
    const saved = raw ? JSON.parse(raw) : { streak: 0, best: 0, lastDate: '' };
    const today = new Date().toDateString();
    const yesterday = new Date(Date.now() - 86400000).toDateString();
    if (saved.lastDate === today) return saved;
    const streak = saved.lastDate === yesterday ? saved.streak + 1 : 1;
    const best = Math.max(saved.best, streak);
    const updated = { streak, best, lastDate: today };
    await AsyncStorage.setItem(STREAK_KEY, JSON.stringify(updated));
    return updated;
  } catch { return { streak: 1, best: 1, lastDate: new Date().toDateString() }; }
}
async function loadScriptStreak(): Promise<{ streak: number; best: number; lastDate: string }> {
  try {
    const raw = await AsyncStorage.getItem(STREAK_KEY);
    return raw ? JSON.parse(raw) : { streak: 0, best: 0, lastDate: '' };
  } catch { return { streak: 0, best: 0, lastDate: '' }; }
}

// ─── UNDO COMMAND PANEL — redesigned ─────────────────────────────────────────
// ─── SCRIPTS LIVE DASH STRIP ─────────────────────────────────────
function ScriptsDashStrip({ historyEntries, isConnected, serverAddr, streak }: {
  ...
  const success = historyEntries.filter(e => e.success).length;
  const pct     = total > 0 ? Math.round((success / total) * 100) : 0;
  const lastRun = historyEntries[0];
  const connColor = isConnected ? '#00FF88' : '#FF5500';
  const pctColor  = pct >= 90 ? '#00FF88' : pct >= 70 ? '#FFB020' : '#FF3131';
  const streakColor = streak.streak >= 7 ? '#00FF88' : streak.streak >= 3 ? '#FFB020' : '#00DCFF';

  // Sparkline from last 12 entries
  const sparkPts = useMemo(() => {
    const last12 = historyEntries.slice(0, 12).reverse();
    if (last12.length < 2) return Array.from({ length: 12 }, () => 20);
    return last12.map(e => e.success ? 80 + (e.ms < 500 ? 15 : 0) : 10);
  }, [historyEntries]);
  ...
  return (
    <View style={sds.card}>
      {/* Top status line */}
      <View style={sds.topRow}>
        <View style={[sds.connChip, { borderColor: connColor + '50', backgroundColor: connColor + '0C' }]}>
          <Animated.View style={{ width: 5, height: 5, borderRadius: 3, backgroundColor: connColor, opacity: pulseAnim }} />
          <Text style={[sds.connTxt, { color: connColor }]}>
            {isConnected ? serverAddr || 'LINKED' : 'OFFLINE'}
          </Text>
        </View>
        {lastRun ? (
          <Text style={sds.lastRunTxt} numberOfLines={1}>
            LAST: {lastRun.scriptName.slice(0, 22)}
          </Text>
        ) : null}
      </View>
      ...
      <View style={sds.statsRow}>
        {/* Stat cells */}
        {[
          { lbl: 'TOTAL RUNS', val: String(total),            col: '#00DCFF' },
          { lbl: 'SUCCESS',    val: `${pct}%`,                col: pctColor  },
          { lbl: 'STREAK',     val: `${streak.streak}d`,      col: streakColor },
          { lbl: 'BEST',       val: `${streak.best}d`,        col: streakColor + 'AA' },
        ].map((s, i) => (
          <View key={i} style={[sds.statCell, i < 3 && { borderRightWidth: 1, borderRightColor: 'rgba(0,220,255,0.12)' }]}>
            <Text style={[sds.statVal, { color: s.col }]}>{s.val}</Text>
            <Text style={sds.statLbl}>{s.lbl}</Text>
          </View>
        ))}

        {/* Mini sparkline */}
        <View style={sds.spark}>
          <Text style={sds.sparkLbl}>RUN HISTORY</Text>
          <View style={{ flexDirection: 'row', alignItems: 'flex-end', height: 28, gap: 1.5, flex: 1 }}>
            {sparkPts.map((pt, i) => {
              const isLast = i === sparkPts.length - 1;
              const col = pt > 50 ? '#00FF88' : '#FF3131';
              const h = Math.max(3, (pt / 100) * 24);
              return (
                <View key={i} style={[{
                  flex: 1, borderRadius: 2,
                  backgroundColor: col,
                  height: h,
                  opacity: isLast ? 1 : 0.35 + (i / sparkPts.length) * 0.55,
...
const sds = StyleSheet.create({
  card:       { backgroundColor: '#0A1420', borderBottomWidth: 1, borderBottomColor: 'rgba(0,220,255,0.15)' },
  topRow:     { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 12, paddingTop: 4, paddingBottom: 3 },
  connChip:   { flexDirection: 'row', alignItems: 'center', gap: 5, borderWidth: 1, borderRadius: 7, paddingHorizontal: 7, paddingVertical: 3 },
  connTxt:    { fontFamily: 'monospace', fontSize: 8.5, fontWeight: '900', letterSpacing: 0.3 },
  lastRunTxt: { fontFamily: 'monospace', fontSize: 8.5, color: 'rgba(100,140,160,0.7)', flex: 1, textAlign: 'right', letterSpacing: 0.3 },
  statsRow:   { flexDirection: 'row', alignItems: 'center', paddingBottom: 4 },
  statCell:   { flex: 1, alignItems: 'center', paddingVertical: 4, paddingHorizontal: 3 },
  statVal:    { fontFamily: 'monospace', fontSize: 15, fontWeight: '900', lineHeight: 18 },
  statLbl:    { fontFamily: 'monospace', fontSize: 6, color: 'rgba(100,140,160,0.6)', letterSpacing: 0.8, marginTop: 1 },
  spark:      { flex: 1.6, paddingHorizontal: 8, paddingVertical: 3, gap: 3 },
  sparkLbl:   { fontFamily: 'monospace', fontSize: 6.5, color: 'rgba(100,140,160,0.5)', letterSpacing: 0.8 },
});
...
function NexusUndoWidget({ isConnected }: { isConnected: boolean }) {
  ...
  const [streak,     setStreak]     = useState<{ streak: number; best: number; lastDate: string }>({ streak: 0, best: 0, lastDate: '' });
  const [pcCount,    setPcCount]    = useState(0);
  const dotPulse = useRef(new Animated.Value(0.4)).current;
  const glowAnim = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    loadScriptStreak().then(setStreak);
    const a = Animated.loop(Animated.sequence([
      Animated.timing(dotPulse, { toValue: 1,   duration: 900, useNativeDriver: true }),
      Animated.timing(dotPulse, { toValue: 0.2, duration: 900, useNativeDriver: true }),
    ]));
    a.start();
    return () => a.stop();
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchUndos = async () => {
      try {
        ...
        const [undoRes, pcRes] = await Promise.allSettled([
          fetch(`http://${ip}:${port}/api/undo/list`, { signal: ctrl.signal }),
          // /api/pc_scripts/list does not exist on server v20 — skip gracefully
          Promise.resolve({ ok: false }),
        ]);
        if (undoRes.status === 'fulfilled' && undoRes.value.ok) {
          const d = await undoRes.value.json();
          const entries = d.entries || [];
          setUndoEntries(entries);
          if (entries.length > 0) setDismissed(false);
        }
        if (pcRes.status === 'fulfilled' && pcRes.value.ok) {
          const d = await pcRes.value.json();
          setPcCount((d.scripts || []).length);
        }
      } catch {}
    };
    fetchUndos();
  ...
  const runCount = undoEntries.length;
  ...
  const handleUndo = () => {
    ...
    Alert.alert(
      'UNDO LAST SCRIPTS',
      `Roll back ${runCount} reversible execution${runCount !== 1 ? 's' : ''}?`,
      [
        ...
        { text: 'UNDO', style: 'destructive', onPress: async () => {
          ...
          try {
            ...
            for (const entry of undoEntries) {
              await fetch(`http://${ip}:${port}/api/undo/rollback`, { method: 'POST', headers, body: JSON.stringify({ id: entry.id }) }).catch(() => {});
            }
  ...
  const streakColor = streak.streak >= 7 ? N.green : streak.streak >= 3 ? N.amber : N.blue;
  const streakEmoji = streak.streak >= 7 ? '🔥' : streak.streak >= 3 ? '⚡' : streak.streak > 0 ? '✓' : '—';

  return (
    <View style={nuw.container}>
      {/* ── UNDO COMMAND ── */}
      <Animated.View style={[nuw.undoCard, {
        borderColor: hasUndo ? N.blue + '60' : N.border,
        ...
        >
          {/* Icon */}
          <View style={[nuw.undoIcon, {
            borderColor: (hasUndo ? N.blue : N.textDim) + '50',
            backgroundColor: (hasUndo ? N.blue : N.textDim) + '12',
          }]}>
            {undoing
              ? <ActivityIndicator size="small" color={N.blue} style={{ transform: [{ scale: 0.75 }] }} />
              : undone
              ? <MaterialIcons name="check-circle" size={16} color={N.green} />
              : <MaterialIcons name="undo" size={16} color={hasUndo ? N.blue : N.textDim + '55'} />
            }
          </View>
          {/* Text */}
          <View style={{ flex: 1 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              <Animated.View style={{ width: 5, height: 5, borderRadius: 3,
                backgroundColor: hasUndo ? N.blue : N.textDim,
                opacity: hasUndo ? dotPulse : 0.3 }} />
              <Text style={[nuw.undoTitle, { color: hasUndo ? '#FFFFFF' : N.textDim }]}>
                {undone ? 'ROLLED BACK' : hasUndo ? 'UNDO READY' : 'UNDO'}
              </Text>
              {hasUndo && (
                <View style={[nuw.undoBadge, { backgroundColor: N.blue }]}>
                  <Text style={nuw.undoBadgeTxt}>{runCount}</Text>
                </View>
              )}
            </View>
            <Text style={[nuw.undoSub, { color: hasUndo ? N.textMid : N.textDim + '60' }]}>
              {undone ? 'Rollback complete' : hasUndo ? `${runCount} exec${runCount !== 1 ? 's' : ''} · 15 min window` : 'Run a script to enable'}
            </Text>
          </View>
          {/* Dismiss */}
          {hasUndo && (
            <TouchableOpacity onPress={(e) => { e.stopPropagation?.(); haptics.light(); setDismissed(true); }}
              hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }} style={nuw.dismiss}>
      ...
      {/* ── STREAK + PC COUNT ── */}
      <View style={nuw.pillRow}>
        {/* Streak pill */}
        <View style={[nuw.pill, { borderColor: streakColor + '50', backgroundColor: streakColor + '0C' }]}>
          <Text style={{ fontSize: 12 }}>{streakEmoji}</Text>
          <View>
            <Text style={[nuw.pillVal, { color: streakColor }]}>{streak.streak}d</Text>
            <Text style={[nuw.pillLbl, { color: streakColor + '80' }]}>STREAK</Text>
          </View>
          <View style={[nuw.pillDivider, { backgroundColor: streakColor + '30' }]} />
          <View>
            <Text style={[nuw.pillVal, { color: streakColor + 'AA', fontSize: 10 }]}>{streak.best}d</Text>
            <Text style={[nuw.pillLbl, { color: streakColor + '60' }]}>BEST</Text>
          </View>
        </View>
        {/* PC live count pill */}
        <View style={[nuw.pill, { borderColor: (isConnected ? N.green : N.textDim) + '40', backgroundColor: (isConnected ? N.green : N.textDim) + '08', flex: 1 }]}>
          <MaterialCommunityIcons name="server" size={14} color={isConnected ? N.green : N.textDim} />
          <View style={{ flex: 1 }}>
            <Text style={[nuw.pillVal, { color: isConnected ? N.green : N.textDim }]}>
              {isConnected ? (pcCount > 0 ? String(pcCount) : '—') : 'OFFLINE'}
            </Text>
            <Text style={[nuw.pillLbl, { color: (isConnected ? N.green : N.textDim) + '70' }]}>PC SCRIPTS</Text>
          </View>
          {isConnected && (
            <Animated.View style={{ width: 5, height: 5, borderRadius: 3, backgroundColor: N.green, opacity: dotPulse }} />
          )}
        </View>
      </View>
    </View>
...
const nuw = StyleSheet.create({
  container:  { marginHorizontal: 16, marginBottom: 8, gap: 6 },
  undoCard:   { borderWidth: 1.5, borderRadius: 11, overflow: 'hidden',
    ...Platform.select({ ios:{ shadowOffset:{width:0,height:4}, shadowOpacity:0.3, shadowRadius:10 }, android:{elevation:4} }) },
  undoInner:  { flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 12, paddingVertical: 10 },
  undoIcon:   { width: 34, height: 34, borderRadius: 9, borderWidth: 1.5, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  undoTitle:  { fontSize: 11, fontWeight: '900', fontFamily: MONO, letterSpacing: 0.5 },
  undoSub:    { fontSize: 9, fontFamily: MONO, marginTop: 1 },
  undoBadge:  { borderRadius: 6, paddingHorizontal: 5, paddingVertical: 2, minWidth: 18, alignItems: 'center' },
  undoBadgeTxt:{ fontSize: 8, fontWeight: '900', color: '#000', fontFamily: MONO },
  dismiss:    { width: 22, height: 22, alignItems: 'center', justifyContent: 'center' },
  pillRow:    { flexDirection: 'row', gap: 6 },
  pill:       { flexDirection: 'row', alignItems: 'center', gap: 8, borderWidth: 1.5, borderRadius: 10,
    paddingHorizontal: 10, paddingVertical: 8 },
  pillVal:    { fontSize: 13, fontWeight: '900', fontFamily: MONO, lineHeight: 16 },
  pillLbl:    { fontSize: 7, fontWeight: '700', fontFamily: MONO, letterSpacing: 0.8 },
  pillDivider:{ width: 1, height: 22, marginHorizontal: 2 },
});
      ...
      <View style={qp.overlay}>
        ...
        <View style={[qp.card, { borderTopColor: col }]}>
          ...
          <View style={[qp.codeHeader, { backgroundColor: N.surfaceHi }]}>
            ...
            <TouchableOpacity onPress={() => { try { require('react-native').Clipboard.setString(code); } catch {}; }} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
              <Text style={{ fontSize: 10, color: N.blue, fontFamily: MONO }}>COPY</Text>
            </TouchableOpacity>
          </View>
...
function ScriptEditorModal({ visible, onClose, onExecute, editScript }: {
  ...
  const handleImport = async () => {
    try {
      const DP = DocumentPicker || (() => { throw new Error('expo-document-picker not available'); })();
  ...
  const handleAskAI = async () => {
    ...
    try {
      ...
      const cleaned = (data.content || data.response || data.message || data.reply || '').replace(/```python\n?/gi, '').replace(/```\n?/g, '').trim();
      if (cleaned) {
            ...
            {seqSlots.length > 0 ? (
              <TouchableOpacity style={{ backgroundColor: N.purple, borderRadius: 12, paddingVertical: 15, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8 }}
                onPress={() => {
                  ...
                  setCode(`# BUTLER SEQUENCER — ${seqSlots.length} steps\n\n` + combined);
            ...
            <ScrollView contentContainerStyle={{ padding: 16, gap: 12, paddingBottom: 120 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              {aiResult ? (
                <View style={{ backgroundColor: N.green + '08', borderRadius: 10, borderWidth: 1, borderColor: N.green + '30', overflow: 'hidden' }}>
                  ...
                  {/* Send to Butler AI — ask AI to explain, improve, or debug the generated script */}
                  <View style={{ flexDirection: 'row', gap: 8, padding: 10, borderTopWidth: 1, borderTopColor: N.green + '20' }}>
                    {[
                      { label: 'EXPLAIN', icon: 'help-outline',    cmd: `Explain what this Python script does step by step:\n\n\`\`\`python\n${aiResult}\n\`\`\`` },
                      { label: 'IMPROVE', icon: 'auto-fix-high',   cmd: `Improve and optimize this Python script, add error handling and comments:\n\n\`\`\`python\n${aiResult}\n\`\`\`` },
                      { label: 'DEBUG',   icon: 'bug-report',       cmd: `Review this Python script for bugs and potential issues:\n\n\`\`\`python\n${aiResult}\n\`\`\`` },
                    ].map(({ label, icon, cmd }) => (
                      <TouchableOpacity
                        key={label}
                        style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, borderWidth: 1.5, borderRadius: 8, paddingVertical: 9, borderColor: N.purple + '50', backgroundColor: N.purple + '0C' }}
                        onPress={() => {
                          haptics.medium();
                          try { (global as any).__butlerInjectMessage?.(cmd); } catch {}
                          try { (global as any).__butlerSwitchTab?.('butler'); } catch {}
                        }}
                        activeOpacity={0.8}
                      >
                        <MaterialIcons name={icon as any} size={11} color={N.purple} />
                        <Text style={{ fontSize: 9, fontWeight: '900', color: N.purple, fontFamily: MONO, letterSpacing: 0.4 }}>{label}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                  <View style={{ paddingHorizontal: 10, paddingBottom: 10 }}>
                    <TouchableOpacity
                      style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderWidth: 1.5, borderRadius: 9, paddingVertical: 11, borderColor: N.green + '60', backgroundColor: N.green + '10' }}
                      onPress={() => {
                        haptics.heavy();
                        const msg = `Here is a Python script I generated:\n\n\`\`\`python\n${aiResult}\n\`\`\`\n\nCan you review it and suggest any improvements?`;
                        try { (global as any).__butlerInjectMessage?.(msg); } catch {}
                        try { (global as any).__butlerSwitchTab?.('butler'); } catch {}
                      }}
                      activeOpacity={0.85}
                    >
                      <MaterialIcons name="send" size={14} color={N.green} />
                      <Text style={{ fontSize: 11, fontWeight: '900', color: N.green, fontFamily: MONO, letterSpacing: 0.5 }}>SEND TO BUTLER AI</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              ) : null}
...
function FavoriteCard({ fav, isRunning, onRun, onUnfav, onDragStart }: {
  ...
  // swipeX drives translateX (native driver) — separate swipeBg drives backgroundColor (JS driver)
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, g) => {
        const v = Math.max(SWIPE_THRESHOLD * 1.4, Math.min(0, g.dx));
        swipeX.setValue(v);
        swipeBg.setValue(v);
      },
      onPanResponderRelease: (_, g) => {
        if (g.dx < SWIPE_THRESHOLD) {
          Animated.spring(swipeX, { toValue: SWIPE_THRESHOLD, tension: 200, friction: 14, useNativeDriver: true }).start();
          Animated.spring(swipeBg, { toValue: SWIPE_THRESHOLD, tension: 200, friction: 14, useNativeDriver: false }).start();
          isSwipedRef.current = true;
        } else {
          Animated.spring(swipeX, { toValue: 0, tension: 220, friction: 14, useNativeDriver: true }).start();
          Animated.spring(swipeBg, { toValue: 0, tension: 220, friction: 14, useNativeDriver: false }).start();
  ...
  const unfavBg = swipeBg.interpolate({ inputRange: [SWIPE_THRESHOLD, 0], outputRange: [N.red + 'FF', N.red + '00'], extrapolate: 'clamp' });
  ...
  return (
    <View style={{ marginBottom: 10, position: 'relative', overflow: 'hidden', borderRadius: 14 }}>
      ...
      <Animated.View style={{ transform: [{ translateX: swipeX as any }, { scale: cardScale }] }} {...panResponder.panHandlers}>
...
// ─── SCRIPTS TIP TICKER — compact single-line strip for the page banner ──────
function ScriptsTipTicker({ tips }: { tips: string[] }) {
  const [idx, setIdx] = useState(0);
  const fadeAnim = useRef(new Animated.Value(1)).current;
  useEffect(() => {
    const t = setInterval(() => {
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 12, paddingVertical: 6, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: 'rgba(0,220,255,0.15)' }}>
      <MaterialIcons name="lightbulb-outline" size={10} color="#00DCFF" />
      <Animated.Text style={{ flex: 1, fontFamily: MONO, fontSize: 9.5, color: '#00DCFFAA', letterSpacing: 0.3, opacity: fadeAnim }} numberOfLines={1}>
        {tips[idx]}
      </Animated.Text>
      <View style={{ borderWidth: 1, borderRadius: 4, borderColor: '#00DCFF30', paddingHorizontal: 5, paddingVertical: 2, backgroundColor: '#00DCFF08' }}>
        <Text style={{ fontFamily: MONO, fontSize: 7.5, color: '#00DCFF70', fontWeight: '700', letterSpacing: 0.5 }}>TIP {idx+1}/{tips.length}</Text>
      </View>
    </View>
  );
}
...
function ScriptsScreenInner() {
  ...
  const [scriptStreak, setScriptStreak]     = useState<{ streak: number; best: number }>({ streak: 0, best: 0 });
  ...
  const fetchServerLibrary = useCallback(async () => {
    ...
    try {
      ...
      // Try both endpoints
      // /api/scripts/library and /api/scripts/list do not exist on server v20
      // Server uses /api/execute for script execution; library is embedded locally
      for (const ep of []) {
        try {
  ...
  const runServerScript = useCallback(async (scriptId: string, scriptName: string) => {
    ...
    try {
      ...
      // /api/scripts/run does not exist on server v20 — use /api/execute
  ...
  const buildScriptFromAI = useCallback(async () => {
    ...
    try {
      ...
      // /api/scripts/build does not exist on server v20 — use butler/chat to build scripts
      const res = await fetch(`http://${ip}:${port}/api/butler/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify({ messages: [{ role: 'user', content: `Write a Python automation script for: ${buildDesc}. Return ONLY the Python code, no explanations, no markdown backticks.` }] }), signal: ctrl.signal });
      const data = await res.json();
      // Server v20 /api/butler/chat returns content, response, or message — not 'script'
      const rawScript = (data.content || data.response || data.message || data.reply || data.script || '').trim();
      // Strip markdown code fences if present
      const cleaned = rawScript.replace(/```python\n?/gi, '').replace(/```\n?/g, '').trim();
      if (cleaned) {
        if (!isValidPythonCode(cleaned)) { throw new Error('AI returned prose instead of code — try a more specific prompt like "Python script that monitors CPU usage"'); }
        setBuildResult({ code: ensureErrorHandling(cleaned), desc: buildDesc }); haptics.success();
      } else throw new Error(data.error || 'AI returned empty response. Make sure Ollama is running on your PC.');
  ...
  const fetchPCLibrary = useCallback(async (query?: string) => {
    ...
    try {
      const path = query ? `/api/kb/search?q=${encodeURIComponent(query)}` : '/api/pc_scripts/list';
      const ctrl = new AbortController();
  ...
  const runPCScript = useCallback(async (scriptId: string, scriptName: string) => {
    ...
    try {
      ...
      // /api/pc_scripts/run does not exist on server v20 — use /api/execute instead
      const res = await fetch(`http://${ip}:${port}/api/execute`, {
        method: 'POST',
  ...
  const savePCScriptToPhone = useCallback(async (script: any) => {
    // /api/pc_scripts/get does not exist on server v20 — scripts are embedded locally
    // Save whatever code is available on the script object directly
    try {
      const code = script.code || script.script || '';
      if (!code) {
        Alert.alert('No Code', 'This script has no code to save.');
        return;
      }
      await saveButlerScript(code, { title: script.name || 'PC Script', category: script.category || 'System' });
      haptics.success();
  ...
  useEffect(() => {
    if (isAppActive) {
      if (!butlerRef.current) butlerRef.current = setInterval(loadButler, 8_000); // 8s saves battery vs 6s
    } else {
  ...
  useEffect(() => {
    ...
    loadScriptStreak().then(setScriptStreak);
    errorAutoFix.invalidateCache();
    loadScriptStreak().then(s => setScriptStreak({ streak: s.streak, best: s.best }));
    ...
    // Do NOT start a second interval here — isAppActive effect already manages butlerRef
    // Starting another interval here causes duplicates. Load once on mount only.
    loadButler();
    return () => {
  ...
  const runScript = useCallback(async (scriptName: string, scriptCode: string, cardId?: string, scriptId?: string, category?: string) => {
    ...
    // Fetch undo entry from server after successful execution so undo button works
    let undoId: number | undefined;
    let undoExpiresSec: number | undefined;
    if (result.success) {
      try {
        const ip = serverConnection.getIP(); const port = serverConnection.getPort();
        if (ip && port) {
          ...
          const undoRes = await fetch(`http://${ip}:${port}/api/undo/list`, {
            headers: serverConnection.getToken() ? { Authorization: `Bearer ${serverConnection.getToken()}` } : {},
            signal: ctrl.signal,
          }).catch(() => null);
          if (undoRes?.ok) {
            const ud = await undoRes.json().catch(() => ({}));
            const entries: any[] = ud.entries ?? [];
            const latest = entries[0];
            if (latest) {
              undoId = latest.id;
              // Parse remaining minutes string like "14 min" -> seconds
              const minsStr: string = String(latest.remainingMin || '');
              const mins = parseFloat(minsStr) || 15;
              undoExpiresSec = Math.round(mins * 60);
            }
          }
        }
      } catch {}
    }
    setExecResult(prev => ({ ...prev, running: false, success: result.success, output: result.output, error: result.error, ms: result.ms, undoId, undoAvailable: !!undoId, undoExpiresSec }));
    ...
    if (result.success) {
      updateScriptStreak().catch(() => {});
      // Refresh run counts and butler scripts after successful execution
      executionCounter.load().then(counts => { setRunCounts(counts); setCachedRunCounts(counts); }).catch(() => {});
    }
  }, []);
  ...
  useFocusEffect(useCallback(() => {
    loadButler();
    loadFavorites().then(favs => { setFavorites(favs); setFavIds(new Set(favs.map(f => f.id))); }).catch(() => {});
  ...
  return (
    <View style={[s.container, { backgroundColor: T.bg || N.bg }]}>
        ...
        extraRow={
          <>
            ...
            <ScriptsDashStrip
              ...
              streak={scriptStreak}
            />
            <ScriptsTipTicker tips={SCRIPTS_PAGE_TIPS} />
          </>
        }
      />
      ...
      {viewMode === 'favorites' ? (
        ...
        <View style={{ flex: 1 }}>
          {/* ── COMMAND STRIP ── */}
          <View style={[s.cmdStrip, { borderBottomColor: PR + '18' }]}>
            {/* Left: FAV + STREAK */}
            <TouchableOpacity
              onPress={() => { haptics.selection(); setViewMode('favorites'); }}
              activeOpacity={0.8}
              style={[s.cmdChip, favIds.size > 0
                ? { borderColor: N.yellow + '70', backgroundColor: N.yellow + '10' }
                : { borderColor: N.textDim + '25', backgroundColor: 'transparent' }]}
            >
              <MaterialIcons name="star" size={13} color={favIds.size > 0 ? N.yellow : N.textDim} />
              <Text style={[s.cmdChipTxt, { color: favIds.size > 0 ? N.yellow : N.textDim }]}>
                {favIds.size > 0 ? favIds.size : 'FAV'}
              </Text>
            </TouchableOpacity>

            {/* Center: count readout — dominant focal point */}
            <View style={[s.cmdCenter, { borderColor: PR + '35', backgroundColor: PR + '07' }]}>
              <View style={{ width: 5, height: 5, borderRadius: 3, backgroundColor: isConnected ? N.green : N.textDim }} />
              <Text style={[s.cmdCountNum, { color: PR }]}>{totalCount + pcLibScripts.length}</Text>
              <Text style={[s.cmdCountUnit, { color: PR + '80' }]}>SCRIPTS</Text>
              <View style={{ flex: 1 }} />
              <Text style={{ fontFamily: MONO, fontSize: 7, color: N.textDim, letterSpacing: 0.5 }}>{isConnected ? 'LIVE' : 'LOCAL'}</Text>
            </View>
            ...
            <View style={{ flexDirection: 'row', gap: 5 }}>
              <TouchableOpacity
                ...
                activeOpacity={0.8}
                style={[s.cmdChip, { borderColor: N.textDim + '25' }]}
                hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
              >
                <MaterialIcons name="history" size={13} color={N.textDim} />
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => { haptics.selection(); setShowChainBuilder(true); }}
                activeOpacity={0.8}
                style={[s.cmdChip, { borderColor: N.green + '40', backgroundColor: N.green + '08' }]}
                hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
              >
                <MaterialIcons name="playlist-play" size={15} color={N.green} />
              </TouchableOpacity>
            </View>
          </View>
          ...
          {/* ── MODE BAR — matches reference image ── */}
          <View style={[s.modeBarWrap, { borderBottomColor: PR + '20' }]}>
            {/* Left: mode tabs */}
            <View style={s.modeTabRow}>
              {([['scripts','code','</> SCRIPTS'],['library','local-library','||| LIBRARY']] as const).map(([m, icon, label]) => {
                const active = viewMode === m;
                return (
                  <TouchableOpacity key={m}
                    style={[s.modeTab, active && { borderColor: PR, backgroundColor: PR + '15' }]}
                    onPress={() => { haptics.selection(); setViewMode(m as ViewMode); }}
                    activeOpacity={0.8}>
                    <MaterialIcons name={icon as any} size={13} color={active ? PR : N.textDim} />
                    <Text style={[s.modeTabTxt, active && { color: PR, fontWeight: '900' }]}>{label}</Text>
                    {active && <View style={{ width: 4, height: 4, borderRadius: 2, backgroundColor: PR }} />}
                  </TouchableOpacity>
                );
              })}
            </View>
            {/* Right: AI Builder + New */}
            <View style={{ flexDirection: 'row', gap: 6 }}>
              <TouchableOpacity
                style={[s.modeIconBtn, { borderColor: N.purple + '50', backgroundColor: N.purple + '0C' }]}
                onPress={() => { haptics.medium(); setBuildResult(null); setBuildDesc(''); setBuildModalVisible(true); }}
                activeOpacity={0.85}>
                <MaterialCommunityIcons name="robot-angry" size={16} color={N.purple} />
              </TouchableOpacity>
              <TouchableOpacity
                style={[s.modeIconBtn, { borderColor: PR + '60', backgroundColor: PR }]}
                onPress={() => { haptics.medium(); setEditingScript(null); setShowEditor(true); }}
                activeOpacity={0.85}>
          ...
          <View style={[s.searchWrap, { backgroundColor: T.panel || '#080E18', borderColor: PR + '35', borderWidth: 1.5 }]}>
            ...
            <TextInput style={[s.searchInput, { color: N.text }]}
              ...
              placeholder={viewMode === 'scripts' ? 'Search scripts, tags, categories...' : 'Search 250+ built-in scripts...'}
            ...
            {(viewMode === 'scripts' ? searchQuery : libSearch) ? (
              ...
              <View style={{ flexDirection: 'row', gap: 4 }}>
                <Text style={{ fontSize: 9, color: N.textDim, fontFamily: MONO, paddingRight: 2 }}>
                  {viewMode === 'scripts' ? `${totalCount} total` : '250+ scripts'}
                </Text>
              </View>
            )}
          </View>
          ...
          {/* ── TIP BANNER ── */}
          {viewMode === 'scripts' && <FuiTipBanner tips={SCRIPTS_PAGE_TIPS} />}
          ...
          {/* PC library section is handled exclusively inside FlatList ListHeaderComponent above */}
...
const s = StyleSheet.create({
  ...
  // ── COMMAND STRIP ─────────────────────────────────────────────────────────
  cmdStrip:   { flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 12, paddingVertical: 5,
    backgroundColor: '#060A12', borderBottomWidth: 1 },
  cmdChip:    { flexDirection: 'row', alignItems: 'center', gap: 5, borderWidth: 1.5, borderRadius: 8,
    paddingHorizontal: 9, paddingVertical: 7 },
  cmdChipTxt: { fontFamily: MONO, fontSize: 10, fontWeight: '900', letterSpacing: 0.3 },
  cmdCenter:  { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 7, borderWidth: 1.5, borderRadius: 9,
    paddingHorizontal: 11, paddingVertical: 7 },
  cmdCountNum:{ fontFamily: MONO, fontSize: 18, fontWeight: '900', lineHeight: 21, letterSpacing: -0.5 },
  cmdCountUnit:{ fontFamily: MONO, fontSize: 7.5, fontWeight: '900', letterSpacing: 1, alignSelf: 'flex-end', marginBottom: 2 },
  ...
  // ── MODE TOGGLE ROW ──────────────────────────────────────────────
  fuiModeRow:   { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 14, paddingTop: 6, paddingBottom: 5 },
  modeBarWrap:  { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 12, paddingVertical: 8,
    backgroundColor: '#040810', borderBottomWidth: 1 },
  modeTabRow:   { flex: 1, flexDirection: 'row', gap: 6 },
  modeTab:      { flexDirection: 'row', alignItems: 'center', gap: 6, borderWidth: 1.5, borderRadius: 20,
    paddingHorizontal: 13, paddingVertical: 9, borderColor: 'rgba(0,255,255,0.15)', backgroundColor: 'transparent' },
  modeTabTxt:   { fontFamily: MONO, fontSize: 11, fontWeight: '700', color: '#3A5068', letterSpacing: 0.3 },
  modeIconBtn:  { width: 38, height: 38, borderRadius: 11, borderWidth: 1.5, alignItems: 'center', justifyContent: 'center' },
  ...
  actionRow:    { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 16, paddingTop: 8, paddingBottom: 0, marginBottom: 0 },
    searchWrap:   { flexDirection: 'row', alignItems: 'center', gap: 10, marginHorizontal: 14, marginBottom: 4, marginTop: 6, borderRadius: 12 },
  searchInput:  { flex: 1, fontSize: 12, color: N.text, fontFamily: MONO, paddingVertical: 0 },
  filterRow:    { paddingHorizontal: 16, marginBottom: 2 },
